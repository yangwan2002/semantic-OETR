...
from .models.backbone import (FeaturePyramidNetwork, MultiScaleResnetEncoder,
                              PatchMerging, ResnetEncoder)
from .models.semantic_fusion import SemanticGuidedFusion
...
class OETREnhanced(nn.Module):
    """OETR with three enhancements for large-scale-difference scenarios:
    ...
    def __init__(self, cfg):
        ...
        # Reuses backbone layer4 (detached) as frozen semantic prior,
        # avoiding a redundant second ResNet forward pass.
    ...
    def feature_extraction(self, image1, image2, mask1=None, mask2=None):
        ...
        Pipeline:
          ...
          4. Semantic branch: reuse backbone layer4 (detached) as frozen
             semantic prior, project and fuse via gated cross-attention
    ...
    def semantic_consistency_loss(self, sem1, sem2, pred_bbox1, pred_bbox2,
                                 h1, w1, h2, w2):
        """Enforce semantic consistency between predicted overlap regions.

        Gradients flow through pred_bbox (soft mask), guiding the regression
        to produce overlap boxes whose semantic content is consistent across
        views. Semantic features are detached so this loss doesn't alter the
        meaning of the frozen semantic prior.
        """
        if sem1 is None or sem2 is None:
            return torch.tensor(0.0, device=pred_bbox1.device)
        ...
        cos_sim = F.cosine_similarity(pooled1, pooled2, dim=-1)
    ...
    def forward_dummy(self, image1, image2, mask1=None, mask2=None):
        ...
        (feat1, feat2, pos1, pos2, hf1, wf1, hf2, wf2,
         _, _, _) = self.feature_extraction(image1, image2, mask1, mask2)
    ...
    def forward(self, data, validation=False):
        ...
        (feat1, feat2, pos1, pos2, hf1, wf1, hf2, wf2,
         sem_feat1, sem_feat2, log_scale) = self.feature_extraction(
            data['image1'][data['overlap_valid']],
            data['image2'][data['overlap_valid']],
            mask1, mask2,
        )
        ...
        if self.use_semantic and self.sem_loss_weight > 0:
            sem_loss = self.semantic_consistency_loss(
                sem_feat1, sem_feat2,
                pred_bbox_xyxy1, pred_bbox_xyxy2,
                h1, w1, h2, w2,
            )
...
def build_detectors(cfg):
    if cfg.MODEL == 'oetr':
        return OETR(cfg)
    elif cfg.MODEL == 'oetr_enhanced':
        return OETREnhanced(cfg)
    else:
